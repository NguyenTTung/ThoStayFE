              ...
              <Fragment key={item.id}>
                <div className="row mb-3">
                  ...
                  <div className="col-6 ps-3">
                    <div className="mb-3">
                      <h4>
                        Trọ: {item.name}{" "}
                        {item.status === 1 && "(Đang chờ duyệt)"}
                      </h4>
                      <div>Địa chỉ: {item.address}</div>
                      <div>
                        Tổng phòng:{" "}
                        {item.roomType.reduce(
                          (sum, room) => sum + room.totalRoom,
                          0
                        )}
                      </div>
                    ...
                    <div className="mb-3">
                      <h4>Loại phòng</h4>
                      {item.roomType.length > 0 &&
                        item.roomType.map((item) => (
                          <Fragment key={item.id}>
                ...
                <div className="d-flex justify-content-end">
                  <button
                    ...
                    onClick={() => handleDeleteMyMotel(item.id)}
                  >
                    Hủy đăng ký trọ
                  </button>
                </div>
